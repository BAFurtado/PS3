...
Prerequisites:
    1. Run prepare_data.py   (produces papers/density_housing_inequality/data/*.csv)
    2. Run regression.py     (produces papers/density_housing_inequality/data/*.csv + regression_table.tex)

Usage (from project root):
    python papers/density_housing_inequality/make_figures.py

Outputs (written to papers/density_housing_inequality/):
    MAIN PAPER:
...
ax.set_xticks([0])
ax.set_xticklabels([rf'Pooled $\Delta Y$ ($N = {len(acp):,}$)'.replace(',', '{,}')])
