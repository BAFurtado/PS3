...
Run from the project root:
    python papers/density_housing_inequality/regression.py
...
STATS_FILES = [SCRIPT_DIR.parent.parent / 'output' / 'final_stats_27cities.csv']
print(f"  Loading {', '.join(p.name for p in STATS_FILES)} "
      f"(this may take a moment)...")

ts = pd.concat([pd.read_csv(p, low_memory=False) for p in STATS_FILES],
               ignore_index=True)
...
# Same seed key as prepare_data.py section 3: the replication index restarts at 0
# in every launch, and Rio de Janeiro was run as two separate launches on two
# machines. Keyed on the bare index the two blocks cross-join against each other's
# baseline. See prepare_data.py for the full note.
_ts_launch = ts['simulation_id'].str.split('__').str[0]
_ts_idx = ts['simulation_id'].str.split('__').str[-1].astype(int)
_ts_key = _ts_launch + '#' + _ts_idx.astype(str)
_order = (pd.DataFrame({'city': ts['processing_acps'], 'key': _ts_key})
          .drop_duplicates().sort_values(['city', 'key']))
_order['seed'] = _order.groupby('city').cumcount()
_seed_map = dict(zip(zip(_order['city'], _order['key']), _order['seed']))
ts['seed'] = list(map(_seed_map.get, zip(ts['processing_acps'], _ts_key)))
