...
Run from the project root:
    python papers/density_housing_inequality/prepare_data.py

Outputs (written to papers/density_housing_inequality/data/):
...
Methodology reference: the manuscript, Sections 3.1–3.5
"""
...
# The batch: 26 cities x 6 configs x 10 seeds, plus Sao Paulo x 6 configs x 6 seeds
# = 1,596 sims. It was run as several launches (the 25-city pack, Rio de Janeiro and
# two Sao Paulo runs, on other machines) and has since been concatenated into one file
# per level; the launch timestamps inside simulation_id survive that, which is what the
# seed key in section 3 relies on.
STATS_FILES = [PROJECT_ROOT / 'output' / 'final_stats_27cities.csv']
REGIONAL_FILES = [PROJECT_ROOT / 'output' / 'regional_stats_27cities.csv']
OUT_DIR      = SCRIPT_DIR / 'data'
OUT_DIR.mkdir(exist_ok=True)

# The submitted-batch panels are archived in data_submitted/ and must not be written
# to: they are the only surviving derived record of the published 27-city, 3-seed run,
# and analysis/validation/compare_density_tier2.py reads them as its baseline.
...
for p in STATS_FILES:
    print(f"  {p.name} ...")
stats = pd.concat([pd.read_csv(p, low_memory=False) for p in STATS_FILES],
                  ignore_index=True)
stats['month'] = pd.to_datetime(stats['month'])
print(f"    {len(stats):,} rows, {stats['simulation_id'].nunique()} simulation_ids")

for p in REGIONAL_FILES:
    print(f"  {p.name} ...")
reg = pd.concat([pd.read_csv(p, low_memory=False) for p in REGIONAL_FILES],
                ignore_index=True)
...
#   <timestamp>__INTEREST_HOUSING=<ih>__POLICY_MELHORIAS=<pm>__PROCESSING_ACPS=<city>__<idx>
#
# The replication index is the last element after splitting on '__', but it is
# NOT a usable seed key on its own: it restarts at 0 in every launch. A city run
# as two separate `-n 5` launches (Rio de Janeiro, on two machines, 2026-08-05
# and 2026-08-07) has two blocks both indexed 0..4, and they are genuinely
# different draws -- their pre-2020 trajectories, which matched seeds would make
# identical, differ by 3-12%. Keyed on the bare index the two blocks cross-join
# against each other's baseline and the city contributes twice the rows it
# should, half of them differenced against the wrong reference trajectory.
#
# The launch timestamp is therefore part of the key. Within a city the
# (timestamp, index) blocks are renumbered 0..k-1 so that `seed < 3` still means
# "the first three replications". Mirrors
# analysis/validation/compare_density_tier2.py::assign_seed.
...
stats['seed'] = assign_seed(stats)
reg['seed']   = assign_seed(reg)

n_launches = (stats.groupby('processing_acps')['simulation_id']
              .apply(lambda s: s.str.split('__').str[0].nunique()))
if (n_launches > 1).any():
    print("  cities run in more than one launch (seed key includes the timestamp):")
    print(n_launches[n_launches > 1].to_string())
...
if seed_check.nunique() > 1:
    print("  WARNING: seed count is not uniform across (city, config):")
    print(seed_check[seed_check != seed_check.max()].to_string())
...
print(f"  reg_mean:   {len(reg_mean)} rows "
      f"(expect {reg.groupby('simulation_id')['mun_id'].nunique().sum()} "
      f"= 1 per sim x municipality)")
...
# Seeds per city are not uniform: 10 in every city but Sao Paulo, which has 6.
n_seeds_total = stats_mean.groupby('processing_acps')['seed'].nunique().sum()

print(f"  Baseline sims (ACP):    {len(baseline_acp)} "
      f"(expect {n_seeds_total} = 1 baseline per city x seed)")
...
print(f"  Treated sims (ACP):     {len(treated_acp)} "
      f"(expect {5 * n_seeds_total} = 5 non-baseline configs per city x seed)")
...
print(f"    (expect {5 * n_seeds_total} = cities x 5 configs x their seeds)")
