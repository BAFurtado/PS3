...
class Output:
    ...
    def save_firms_data(self, sim):
        ...
        for firm in sim.firms.values():
            ...
            firms_data['total_balance'].append(float(firm.total_balance))
            firms_data['number_employees'].append(firm.num_employees)
            firms_data['stocks'].append(float(firm.total_quantity))
            firms_data['amount_produced'].append(firm.amount_produced)
            firms_data['price'].append(float(firm.prices))
            firms_data['amount_sold'].append(firm.amount_sold)
            firms_data['revenue'].append(float(firm.revenue))
            firms_data['profit'].append(float(firm.profit))
            firms_data['wages_paid'].append(float(firm.wages_paid))
            firms_data['input_cost'].append(float(firm.input_cost))
            firms_data['emissions'].append(float(firm.last_emissions))
            firms_data['eco_eff'].append(float(firm.env_efficiency))
            ...
            if firm.sector == 'Construction':
                ...
                construction_data['total_balance'].append(float(firm.total_balance))
                construction_data['number_employees'].append(firm.num_employees)
                construction_data['stocks'].append(float(firm.total_quantity))
                construction_data['amount_produced'].append(len(firm.houses_built))
                construction_data['price'].append(float(firm.mean_house_price()))
