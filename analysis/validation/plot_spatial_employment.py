...
def plot_mun_hist(real,sim, text, full_region, urban_region):
    real = real.sort_values(by='qtde_vinc_ativos_sum', ascending=False)
    sim = sim.sort_values(by='employment', ascending=False)
    real['emp'] = real['qtde_vinc_ativos_sum']/np.mean( real['qtde_vinc_ativos_sum'])#(real['qtde_vinc_ativos_sum']-np.mean(real['qtde_vinc_ativos_sum']))/np.std(real['qtde_vinc_ativos_sum'])
    sim['emp'] = sim['employment']/np.mean(sim['employment'])#(sim['employment']-np.mean(sim['employment']))/np.std(sim['employment'])
    ...
    fig, ax = plt.subplots(figsize=(15, 15), subplot_kw={'aspect': 'equal'})
    ax.bar(x - bar_width / 2, sim['emp'], width=bar_width, label='Simulated Data', color='blue')
    ax.bar(x + bar_width / 2, real['emp'], width=bar_width, label='Real Data', color='green')

    ax.grid(True, color='grey', linestyle='-')
    title = f'Normalized employement Distribution'
    ax.set_title(title)
    ...
    #plt.xlabel('Rank')
    plt.ylabel('Normalized total employement per region')
...
if __name__ == '__main__':
    ...
    cols_s = ['mun_id', 'regional_unemployment', 'pop']
